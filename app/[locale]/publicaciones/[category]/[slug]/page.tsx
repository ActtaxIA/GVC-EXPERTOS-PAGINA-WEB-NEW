...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `${siteConfig.url}/${params.locale}/publicaciones/${params.category}/${post.slug}`,
    },
    openGraph: {
      type: 'article',
      title: post.title,
...
export default async function ArticlePage({
  ...
  const shareUrl = `${siteConfig.url}/${params.locale}/publicaciones/${params.category}/${post.slug}`
        ...
        <div className="container-custom">
          <Breadcrumbs
            items={[
              ...
              { label: post.category?.name || params.category, href: `/publicaciones/${params.category}` },
              { label: post.title },
            ]}
          ...
          {post.category && (
            <LocalizedLink 
              href={`/publicaciones/${params.category}`}
              className="inline-block px-3 py-1 bg-gold/20 text-gold text-sm font-semibold rounded mb-4"
            >
              {post.category.name}
            </LocalizedLink>
          )}
                ...
                <div className="bg-white p-6 rounded-sm shadow-sm">
                  ...
                  <div className="space-y-4">
                    {relatedPosts.map((relPost: any) => (
                      <LocalizedLink key={relPost.id} href={`/publicaciones/${relPost.categorySlug}/${relPost.slug}`} className="flex gap-3 group">
