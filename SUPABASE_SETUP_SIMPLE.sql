-- =====================================================
-- GVC EXPERTOS - CONFIGURACIÓN SUPABASE (SIMPLIFICADA)
-- =====================================================
-- Copia y pega este script completo en el SQL Editor de Supabase
-- Usamos Supabase Auth en lugar de tabla custom de usuarios
-- =====================================================
...
-- Crear políticas de LECTURA pública (cualquiera puede ver el contenido publicado)
...
CREATE POLICY "Team members are viewable by everyone" ON public.team_members FOR SELECT USING (is_active = true);
...
CREATE POLICY "Published success cases are viewable by everyone" ON public.success_cases FOR SELECT USING (is_published = true);
...
CREATE POLICY "Hospitals are viewable by everyone" ON public.hospitals FOR SELECT USING (is_active = true);

-- Política para que CUALQUIERA pueda enviar formularios de contacto (leads)
...
-- =====================================================
-- SIGUIENTE PASO:
-- 1. Ve a Authentication > Providers en Supabase y habilita Email
-- 2. Ve a Authentication > Users y crea un usuario admin manualmente
-- 3. Ve a Storage y crea un bucket llamado "images" (público)
-- 4. ¡Listo para el deploy!
